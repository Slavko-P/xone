...
#define GIP_GR_NAME "Mad Catz Rock Band 4 Stratocaster"

enum gip_guitar_button {
	GIP_GR_BTN_MENU = BIT(2),
	GIP_GR_BTN_VIEW = BIT(3),
	GIP_GR_BTN_DPAD_U = BIT(8),
	GIP_GR_BTN_DPAD_D = BIT(9),
	GIP_GR_BTN_DPAD_L = BIT(10),
	GIP_GR_BTN_DPAD_R = BIT(11),
};

enum gip_guitar_fret {
	GIP_GR_FRET_GREEN = BIT(0),
	GIP_GR_FRET_RED = BIT(1),
	GIP_GR_FRET_YELLOW = BIT(2),
	GIP_GR_FRET_BLUE = BIT(3),
	GIP_GR_FRET_ORANGE = BIT(4),
};
...
static int gip_guitar_op_input(struct gip_client *client, void *data, int len)
{
	struct gip_guitar *guitar = dev_get_drvdata(&client->dev);
	...
	input_report_key(dev, BTN_START, buttons & GIP_GR_BTN_MENU);
	input_report_key(dev, BTN_SELECT, buttons & GIP_GR_BTN_VIEW);
	input_report_key(dev, BTN_TRIGGER_HAPPY1,
			 pkt->fret_upper & GIP_GR_FRET_GREEN);
	input_report_key(dev, BTN_TRIGGER_HAPPY2,
			 pkt->fret_upper & GIP_GR_FRET_RED);
	input_report_key(dev, BTN_TRIGGER_HAPPY3,
			 pkt->fret_upper & GIP_GR_FRET_YELLOW);
	input_report_key(dev, BTN_TRIGGER_HAPPY4,
			 pkt->fret_upper & GIP_GR_FRET_BLUE);
	input_report_key(dev, BTN_TRIGGER_HAPPY5,
			 pkt->fret_upper & GIP_GR_FRET_ORANGE);
	input_report_key(dev, BTN_TRIGGER_HAPPY6,
			 pkt->fret_lower & GIP_GR_FRET_GREEN);
	input_report_key(dev, BTN_TRIGGER_HAPPY7,
			 pkt->fret_lower & GIP_GR_FRET_RED);
	input_report_key(dev, BTN_TRIGGER_HAPPY8,
			 pkt->fret_lower & GIP_GR_FRET_YELLOW);
	input_report_key(dev, BTN_TRIGGER_HAPPY9,
			 pkt->fret_lower & GIP_GR_FRET_BLUE);
	input_report_key(dev, BTN_TRIGGER_HAPPY10,
			 pkt->fret_lower & GIP_GR_FRET_ORANGE);
	input_report_abs(dev, ABS_X, pkt->slider);
	input_report_abs(dev, ABS_Y, pkt->whammy);
	input_report_abs(dev, ABS_Z, pkt->tilt);
	input_report_abs(dev, ABS_HAT0X, !!(buttons & GIP_GR_BTN_DPAD_R) -
					 !!(buttons & GIP_GR_BTN_DPAD_L));
	input_report_abs(dev, ABS_HAT0Y, !!(buttons & GIP_GR_BTN_DPAD_D) -
					 !!(buttons & GIP_GR_BTN_DPAD_U));
	input_sync(dev);
...
static int gip_guitar_probe(struct gip_client *client)
{
	...
	err = gip_init_input(&guitar->input, client, GIP_GR_NAME);
	if (err)
		return err;
...
static void gip_guitar_remove(struct gip_client *client)
{
	struct gip_guitar *guitar = dev_get_drvdata(&client->dev);
